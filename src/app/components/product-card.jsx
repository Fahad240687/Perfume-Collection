...
export default function ProductCard({ product }) {
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) {
        stars.push(<FaStar key={i} className="text-amber-400" />) // Filled star
      } else {
        stars.push(<FaRegStar key={i} className="text-gray-500" />) // Empty star
      }
    }
    return stars
  }
  ...
  return (
<div className="w-full min-h-[420px] bg-[#1F1F1F] rounded-sm overflow-hidden flex flex-col group">
