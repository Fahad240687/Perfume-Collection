...
import { FaStar, FaRegStar, FaHeart, FaMinus, FaPlus, FaShoppingBag, FaShoppingCart } from "react-icons/fa";
 ...
 const [animateHeading, setAnimateHeading] = useState(false);
  ...
  const fetchDiscoverProducts = async () => {
    try {
      ...
      console.log("Discover products data:", data); // Add this line
      if (data.success) {
...
const handleOrderNow = () => {
  if (product) {
    ...
    openCartPopup(); // This will trigger popup (or you can redirect to /cart)
  }
};
  if (loading) {
    return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 w-full max-w-[1800px] mx-auto px-4 sm:px-6">
        {discoverProducts.map((item) => (
          ...
          >
            ...
            <div 
              className="border border-[#CE9F56] rounded-t-xl overflow-hidden cursor-pointer h-64" // Added fixed height h-64
            ...
            >
              <div className="relative w-full h-full"> {/* Changed from aspect-square to h-full */}
                <Image
            ...
            <div className="p-4 rounded-b-xl flex-grow">
              ...
              <div className="flex justify-between items-center mb-2">
                <p className="text-[#DAB060] font-bold text-md">
                  ${item.price.toFixed(2)}
                </p>
              </div>
              
              {/* Rating moved to next line */}
              <div className="flex justify-start gap-1 text-[#DAB060] mb-3">
